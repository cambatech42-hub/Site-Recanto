      ...
      <div className="container mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-2">
            <h3 className="text-2xl font-serif font-bold mb-4">Pousada Recanto do Lago</h3>
            <p className="text-background-beige/80 max-w-md">
              Seu refúgio de paz e aventura no coração de Cambará do Sul. Viva a natureza, o conforto e a hospitalidade da Serra Gaúcha.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {NAV_LINKS.map(link => (
                <li key={link.name}>
                  <a href={link.href} className="text-background-beige/80 hover:text-white transition-colors">{link.name}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-background-beige/20 mt-8 pt-6 text-center text-sm text-background-beige/70">
          <p>&copy; {currentYear} Pousada Recanto do Lago. Todos os direitos reservados.</p>
        </div>
      </div>
    </footer>
  );
